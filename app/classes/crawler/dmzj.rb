...
class Crawler::Dmzj
  ...
  def crawl_novel(category_id)
    img_link = @page_html.css("#cover_pic")[0][:src]
    name = @page_html.css(".con h3")[0].text
    is_serializing = false
    is_serializing = true if @page_html.css(".con p")[2].text.index("连载中")
    author = @page_html.css(".con p")[0].text.gsub("作者：","")
    description = change_node_br_to_newline(@page_html.css(".ilist p")).strip
    link = @page_url
